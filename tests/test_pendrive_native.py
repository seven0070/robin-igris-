"""Pendrive-native engine — hardware-up, not transformer."""
...
def test_idle_and_overnight_metabolism(tmp_path: Path):
    ...
    assert night["synthesized"] >= 1  # capital_cities abstraction
...
def test_kernel_wires_pne(tmp_path: Path, monkeypatch):
    ...
    assert "Pendrive-native model" in k.boot_context()


def test_shell_pne(tmp_path: Path, monkeypatch):
    (tmp_path / ".robin_usb").touch()
    monkeypatch.setenv("ROBIN_USB_ROOT", str(tmp_path))
    from aos.shell import AgentShell

    shell = AgentShell(AgentKernel.create(tmp_path))
    st = json.loads(shell.handle_line(":pne"))
    assert st["architecture"] == "pendrive-native-v0"
    teach = json.loads(shell.handle_line(":pne Madrid is the capital of Spain."))
    assert teach["found"] is True
    ask = json.loads(shell.handle_line(":pne What is the capital of Spain?"))
    assert "madrid" in ask["reply"].lower()


def test_pne_tools(tmp_path: Path, monkeypatch):
    assert "pne_ask" in TOOL_IMPLS
    (tmp_path / ".robin_usb").touch()
    monkeypatch.setenv("ROBIN_USB_ROOT", str(tmp_path))
    out = json.loads(run_tool("pne_ask", {"text": "Your name is Carry."}))
    assert "carry" in out["reply"].lower()
